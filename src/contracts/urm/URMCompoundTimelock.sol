...
contract URMCompoundTimelock is URMCore {
  ...
  function _queue(
    ...
    for (uint256 _i = 0; _i < _targets.length; _i++) {
      ICompoundTimelockTarget(TARGET).queueTransaction(
        _targets[_i], _values[_i], "", _calldatas[_i], _rollback.executableAt
      );
    }
  }
  ...
  function _cancel(
    ...
    for (uint256 _i = 0; _i < _targets.length; _i++) {
      ICompoundTimelockTarget(TARGET).cancelTransaction(
        _targets[_i], _values[_i], "", _calldatas[_i], _rollback.executableAt
      );
    }
  }
  ...
  function _execute(
    ...
    for (uint256 _i = 0; _i < _targets.length; _i++) {
      ICompoundTimelockTarget(TARGET).executeTransaction(
        _targets[_i], _values[_i], "", _calldatas[_i], _rollback.executableAt
      );
    }
  }
