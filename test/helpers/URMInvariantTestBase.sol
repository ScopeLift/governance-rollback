...
/// @title URMInvariantTestBase
/// @notice Base class for URM invariant tests that provides common invariant logic
/// @dev This abstract class contains all the invariant checks that are identical between
///      URMCompoundManager and URMOZManager. Subclasses need to implement:
///      - setUp() - to initialize the specific URM contract and handler
///      - _getURM() - to return the URM contract instance
///      - _getHandler() - to return the handler instance
abstract contract URMInvariantTestBase is Test {
  ...
  function _checkExpiredProposalNotExecutable(RollbackProposal memory _proposal) external {
    vm.prank(guardian);
    try _getURM().execute(_proposal.targets, _proposal.values, _proposal.calldatas, _proposal.description) {
  ...
  function _checkExpiredProposalNotCancelable(RollbackProposal memory _proposal) external {
    vm.prank(guardian);
    try _getURM().cancel(_proposal.targets, _proposal.values, _proposal.calldatas, _proposal.description) {
  ...
  function _checkExpiredProposalNotQueueable(RollbackProposal memory _proposal) external {
    vm.prank(guardian);
    try _getURM().queue(_proposal.targets, _proposal.values, _proposal.calldatas, _proposal.description) {
