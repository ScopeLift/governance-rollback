...
/// @title URMHandlerBase
/// @notice Base handler contract for URM invariant testing
/// @dev This abstract class contains all the common functionality between
///      URMCompoundManager and URMOZManager handlers. Subclasses need to implement:
///      - _getURM() - to return the URM contract instance
///      - _getDescription() - to return the description string
abstract contract URMHandlerBase is Test {
  ...
  constructor(address _admin, address _guardian, FakeProtocolContract[] memory _targets) {
    ...
    // Note: URM address will be set by child class after initialization
  }

  /// @notice Abstract method to get the URM contract instance
  /// @return The URM contract instance
  ...
  /// @notice Set the URM address in the RollbackSet (called by child constructor)
  function _setURMAddress() internal {
    _rollbackSet.setURM(address(_getURM()));
  }
  ...
  function propose(uint256 _rollbackFee) external countCall("propose") {
    ...
    // Get the rollback ID
    uint256 _rollbackId = _getURM().getRollbackId(_targets, _values, _calldatas, _getDescription());
    ...
    vm.prank(admin);
    _getURM().propose(_targets, _values, _calldatas, _getDescription());
  ...
  function wrapBeforeExpiryAndQueue(uint256 _randomIndex) external countCall("wrapBeforeExpiryAndQueue") {
    ...
    uint256 _queueExpiresAt = _getURM().getRollback(_randomProposal.rollbackId).queueExpiresAt;
  ...
  function warpAndExecute(uint256 _randomIndex) external countCall("warpAndExecute") {
    ...
    uint256 _executableAt = _getURM().getRollback(_randomProposal.rollbackId).executableAt;
