...
contract URMCompoundManagerIntegrationTest is Test, DeployInput {
  ...
  function setUp() public {
    ...
    fakeProtocolContract = new FakeProtocolContract(urm.admin());
...
contract ProposeWithRollback is URMCompoundManagerIntegrationTest {
  ...
  function testFork_RollbackExecutionFlow() public {
    ...
    // 5. Queue rollback
    vm.prank(GUARDIAN);
    uint256 _rollbackId = urm.queue(targets, values, calldatas, description);

    // 5. Wait for timelock delay and execute rollback
    uint256 timelockDelay = ICompoundTimelock(payable(urm.TARGET_TIMELOCK())).delay();
    ...
    assertEq(uint8(urm.state(_rollbackId)), uint8(ProposalState.Executed));
  }
